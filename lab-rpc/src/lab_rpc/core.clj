...
(defrecord ClientEnd [end-name ch done]
  ...
  (call! [_ svc-meth args]
    (let [req    (->ReqMsg end-name svc-meth args (async/chan))
          result (async/alt!!
                   [[ch req]] true
                   done nil)]
      (when result
        (let [{:keys [reply]} (async/<!! (:reply-ch req))]
          reply)))))
...
(defn make-end*
  [{:keys [ends] :as state} end-name client-end]
      ...
      (if (and enabled
        ...
        (do
          ...
          (if (and (not reliable)
            ...
            (async/put! reply-ch (->ReplyMsg false nil))
            ...
            (let [ech (async/chan)]
              ...
              (let [[reply reply-ok]
                    (loop []
                      (if-let [reply (async/alt!!
                                       ech ([r] r)
                                       (async/timeout 100) nil)]
                        [reply true]
                        (if (server-dead? this end-name server-name server)
                          (do
                            (async/thread (async/<!! ech))
                            [nil false])
                          (recur))))
                    server-dead (server-dead? this end-name server-name server)]
                (cond
                  (or (not reply-ok)
                      server-dead)
        ...
        (let [ms (if (:long-delays @state)
                   (rand-int 7000)
                   (rand-int 100))]
          (time/after-func ms #(async/put! reply-ch (->ReplyMsg false nil)))))))
        ...
        (loop []
          (when-let [req (async/alt!!
            ...
            (swap! state update :bytes (partial + buffer-header-size (-> (:args req)
                                                                   ->bytes
                                                                   count)))
            (async/thread (process-request rn req))
            (recur)))))
    rn))
